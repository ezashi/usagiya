class Order < ApplicationRecord
  ...
  validates :phone, presence: true, format: { with: /\A\d{2,4}-?\d{2,4}-?\d{3,4}\z/ }
  ...
  # 包装のバリデーション
  validates :wrapping_type, presence: true, inclusion: { in: %w[no_wrapping logo_red logo_blue rabbit_pink] }

  # 送り先のバリデーション（別の住所の場合のみ）
  validates :delivery_name, presence: true, length: { maximum: 50 }, if: :different_shipping_address?
  validates :delivery_postal_code, presence: true, format: { with: /\A\d{3}-?\d{4}\z/ }, if: :different_shipping_address?
  validates :delivery_address, presence: true, length: { maximum: 200 }, if: :different_shipping_address?
  validates :delivery_phone, presence: true, format: { with: /\A\d{2,4}-?\d{2,4}-?\d{3,4}\z/ }, if: :different_shipping_address?
